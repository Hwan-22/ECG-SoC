...
- 1단계: OA1, OA2 전치증폭기 — 차동 신호 증폭(R_fb=100kΩ, Rg=1kΩ), 공통모드 통과
- 2단계: OA3 차동증폭기 — 공통모드 제거, R5=R7=R8=R9=10kΩ (이득 1, 비율 매칭으로 CMRR 확보)

**HPF (고역통과 필터)**
- 차단 주파수 fc ≈ 0.48 Hz (C=33nF, R=10MΩ) — DC 오프셋(±300mV) 제거
- 허용 DC 오프셋: IEC 60601-2-47 기준 ±300mV

**Notch 필터 (능동 Twin-T)**
- 60 Hz 전원선 간섭 제거 — 부트스트랩(Q≈5)으로 통과대역 보존
- XModel 측정: 60Hz 80dB 제거, 통과대역 −3dB = 150Hz

**XModel+Questa 측정 검증값** (`scripts/run_char.sh`, `run_pli.sh`)
| 항목 | 목표 | 측정 |
|---|---|---|
| 통과대역 이득 | 201 | ~200 |
| −3dB 대역폭 | 150Hz | 150Hz |
...
*모델은 저항 이상매칭 가정; 실제 CMRR은 저항 오차(예: 0.1%→~66dB)로 제한.
